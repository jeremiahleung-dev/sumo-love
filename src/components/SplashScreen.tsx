...
export default function SplashScreen() {
  ...
  const inputClass = "w-full bg-[#18181B] border border-[#27272A] rounded-lg px-4 py-3 text-sm text-[#FAFAFA] placeholder-[#3F3F46] focus:outline-none focus:border-[#D4A97A] transition-colors";

  return (
    <div className="fixed inset-0 bg-[#09090B] z-[100] flex flex-col items-center justify-center overflow-hidden px-6">

      {/* ── Dohyo ring + rikishi ─────────────────────────────────── */}
      <motion.svg viewBox="0 0 220 220" className="w-64 h-64" aria-hidden>
        <motion.circle
          cx="110" cy="110" r="100"
      ...
      {/* ── Kanji + brand ────────────────────────────────────────── */}
      <motion.div
        className="mt-3 font-display font-black text-5xl text-[#FAFAFA] leading-none select-none"
      ...
      <motion.p
        className="mt-1.5 font-display font-semibold text-sm tracking-[0.35em] text-[#D4A97A] uppercase"
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <input
            ...
            className={inputClass}
          />
          {/* Password row: input + Google icon button side by side */}
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-none flex items-center justify-center w-12 h-12 bg-[#18181B] border border-[#27272A] rounded-lg hover:border-[#D4A97A] transition-colors cursor-pointer"
            >
              <GoogleIcon />
            </button>
          </div>

          {error && <p className="text-[#C0292A] text-xs text-center">{error}</p>}

          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#C0292A] hover:bg-[#A82424] disabled:opacity-50 text-white py-3 rounded-lg font-semibold text-sm transition-colors duration-200 cursor-pointer"
          >
        ...
        <button
          ...
          className="w-full mt-3 text-[#52525B] hover:text-[#A1A1AA] text-xs transition-colors cursor-pointer text-center"
        >
